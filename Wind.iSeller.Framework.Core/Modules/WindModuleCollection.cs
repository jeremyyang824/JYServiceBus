...
{
    /// <summary>
    /// Used to store WindModuleInfo objects as a dictionary.
    /// </summary>
    internal class WindModuleCollection : List<WindModuleInfo>
    {
        public Type StartupModuleType { get; private set; }

        public WindModuleCollection(Type startupModuleType)
        {
        ...
        public TModule GetModule<TModule>() where TModule : WindModule
        {
            var module = this.FirstOrDefault(m => m.Type == typeof(TModule));
            if (module == null)
            {
                throw new WindException("Can not find module for " + typeof(TModule).FullName);
            }
        ...
        public List<WindModuleInfo> GetSortedModuleListByDependency()
        {
        ...
        public static void EnsureKernelModuleToBeFirst(List<WindModuleInfo> modules)
        {
            var kernelModuleIndex = modules.FindIndex(m => m.Type == typeof(WindKernelModule));
        ...
        public static void EnsureStartupModuleToBeLast(List<WindModuleInfo> modules, Type startupModuleType)
        {
